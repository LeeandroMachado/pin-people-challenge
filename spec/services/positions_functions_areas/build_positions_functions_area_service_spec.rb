...
RSpec.describe PositionsFunctionsAreas::BuildPositionsFunctionsAreaService, type: :service do
  let(:service) { described_class.new }

  let(:caches) { { functions: {}, functional_areas: {}, positions: {} } }
  let(:row) do
    {
      nome: "Demo 001",
  ...
  describe "#call" do
    context "with valid parameters" do
      it "creates a valid new PositionsFunctionsArea instance" do
        result = service.call(row, caches)

        expect(result.valid?).to be_truthy
        expect(result.class).to eq(PositionsFunctionsArea)
      end
    end
  end
end
